...
<template v-else-if="exampleId === 2">
  ...
  // hasło: numer indeksu
  Backend.userLogin('stuXXXXX', 'XXXX')
...
const examples = [
  ...
  () => {
    const studentUsr = "stu15033";
    const studentPass = "15033";
    if (!studentUsr || !studentPass)
      alert("Uzupełnij dane do logowania studenta w widoku TestView.vue linia:74-75");
    Backend.userLogin(studentUsr, studentPass).then((r) => (result.value = r));
  },
  () =>
